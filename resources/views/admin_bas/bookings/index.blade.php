...
<div class="flex flex-col gap-6 h-full">
    ...
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('bas.bookings.export') }}" class="flex cursor-pointer items-center justify-center overflow-hidden rounded-full h-10 px-4 bg-secondary text-dark gap-2 text-sm font-bold leading-normal tracking-[0.015em]">
            Export CSV
        </a>
    </div>
                ...
                <tbody id="bookingsTableBody">
                    ...
                    <tr class="border-b border-border booking-row" 
                        ...
                        <td class="px-4 py-4 text-right">
                            <a href="{{ route('bas.bookings.edit', $booking->id) }}" class="text-accent hover:underline mr-2">Edit</a>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        searchInput.addEventListener('input', function() {
            ...
            bookingRows.forEach(row => {
                const searchText = (row.dataset.room + ' ' + row.dataset.organizer).toLowerCase();
