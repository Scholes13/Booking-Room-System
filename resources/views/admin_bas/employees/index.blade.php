...
<div class="flex flex-col gap-6 h-full">
    ...
    <div class="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
        <div>
            <h1 class="text-dark text-2xl font-bold leading-tight tracking-[-0.015em]">Employees</h1>
            <p class="text-gray-500 mt-1">View and manage all employees</p>
        </div>
        <button id="btnAddEmployee" class="flex cursor-pointer items-center justify-center overflow-hidden rounded-lg h-10 px-5 bg-[#24448c] text-white gap-2 text-sm font-bold leading-normal tracking-[0.015em] hover:bg-[#1c3670] transition-colors shadow-sm w-full sm:w-auto">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        ...
        <div class="bg-white rounded-lg p-4 md:p-5 shadow-sm border border-gray-200 hover:shadow-md transition-shadow duration-300">
            <div class="flex items-center gap-3">
                <div class="flex-shrink-0 flex justify-center items-center rounded-full bg-[#24448c] bg-opacity-10 p-2 md:p-3">
        ...
        <div class="bg-white rounded-lg p-4 md:p-5 shadow-sm border border-gray-200 hover:shadow-md transition-shadow duration-300">
            <div class="flex items-center gap-3">
                ...
                <div class="flex-1 min-w-0">
                    <h2 class="text-sm text-gray-500 font-medium">Male</h2>
        ...
        <div class="bg-white rounded-lg p-4 md:p-5 shadow-sm border border-gray-200 hover:shadow-md transition-shadow duration-300">
            <div class="flex items-center gap-3">
                ...
                <div class="flex-1 min-w-0">
                    <h2 class="text-sm text-gray-500 font-medium">Female</h2>
        ...
        <div class="bg-white rounded-lg p-4 md:p-5 shadow-sm border border-gray-200 hover:shadow-md transition-shadow duration-300">
            <div class="h-full flex flex-col">
                <div class="flex items-center gap-3 mb-3">
                    ...
                    <div class="flex-1 min-w-0">
                        <h2 class="text-sm text-gray-500 font-medium">Employee Data</h2>
                        <p class="text-base font-medium text-dark">Export Excel</p>
                    </div>
                </div>
                <a href="{{ route('bas.employees.export') }}" class="mt-auto py-2 px-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors shadow-sm text-sm font-medium flex items-center justify-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
    ...
    <div class="flex flex-col gap-4 bg-white rounded-lg p-4 md:p-5 shadow-sm">
        <h2 class="text-base font-semibold text-gray-700">Filter Employees</h2>
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <!-- Search Input with Responsive Design -->
            <div class="relative w-full">
                <form action="{{ route('bas.employees') }}" method="GET">
                    <div class="absolute inset-y-0 start-0 flex items-center ps-3 pointer-events-none">
                        <svg class="w-4 h-4 text-gray-500" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 20">
                            <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m19 19-4-4m0-7A7 7 0 1 1 1 8a7 7 0 0 1 14 0Z"/>
                        </svg>
                    </div>
                    <input type="search" id="searchInput" name="search" value="{{ request('search') }}" class="block w-full p-3 ps-10 text-sm text-dark border border-gray-200 rounded-lg bg-gray-50 focus:ring-[#24448c] focus:border-[#24448c] transition-all shadow-sm" placeholder="Search name, position, phone, or email...">
                </form>
            </div>
            
            <!-- Department Filter -->
            <div>
                <select id="departmentFilter" name="department_id" class="block w-full p-3 text-sm text-dark border border-gray-200 rounded-lg bg-gray-50 focus:ring-[#24448c] focus:border-[#24448c] transition-all shadow-sm appearance-none bg-no-repeat bg-[right_0.5rem_center] bg-[length:1em]" style="background-image: url('data:image/svg+xml;charset=UTF-8,%3csvg xmlns=%27http://www.w3.org/2000/svg%27 fill=%27none%27 viewBox=%270 0 20 20%27%3e%3cpath stroke=%27%236B7280%27 stroke-linecap=%27round%27 stroke-linejoin=%27round%27 stroke-width=%271.5%27 d=%27M6 8l4 4 4-4%27/%3e%3c/svg%3e')">
            ...
            <!-- Gender Filter -->
            <div>
                <select id="genderFilter" name="gender" class="block w-full p-3 text-sm text-dark border border-gray-200 rounded-lg bg-gray-50 focus:ring-[#24448c] focus:border-[#24448c] transition-all shadow-sm appearance-none bg-no-repeat bg-[right_0.5rem_center] bg-[length:1em]" style="background-image: url('data:image/svg+xml;charset=UTF-8,%3csvg xmlns=%27http://www.w3.org/2000/svg%27 fill=%27none%27 viewBox=%270 0 20 20%27%3e%3cpath stroke=%27%236B7280%27 stroke-linecap=%27round%27 stroke-linejoin=%27round%27 stroke-width=%271.5%27 d=%27M6 8l4 4 4-4%27/%3e%3c/svg%3e')">
                    <option value="">All Genders</option>
                    <option value="L" {{ request('gender') == 'L' ? 'selected' : '' }}>Male</option>
                    <option value="P" {{ request('gender') == 'P' ? 'selected' : '' }}>Female</option>
                </select>
            </div>
        </div>
    </div>
...
<div id="confirmDeleteModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50 transition-opacity duration-300">
    <div class="bg-white p-6 rounded-xl shadow-xl max-w-md w-full mx-4 transform transition-transform duration-300 scale-100">
        ...
        <p class="text-gray-600 text-center mb-6">Are you sure you want to delete this employee? This action cannot be undone.</p>
...
<div id="editEmployeeModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50 transition-opacity duration-300">
    <div class="bg-white p-6 rounded-xl shadow-xl max-w-3xl w-full mx-4 transform transition-transform duration-300 scale-100">
        ...
        <form action="{{ route('bas.employees.update', '') }}" id="editEmployeeForm" method="POST">
            @csrf
            @method('PUT')
...
<div id="addEmployeeModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50 transition-opacity duration-300">
    <div class="bg-white p-6 rounded-xl shadow-xl max-w-3xl w-full mx-4 transform transition-transform duration-300 scale-100">
        ...
        <form action="{{ route('bas.employees.store') }}" method="POST">
            @csrf
    ...
    function initializeDeleteButtons() {
        document.querySelectorAll('.delete-employee').forEach(button => {
            button.addEventListener('click', function() {
                ...
                document.getElementById('deleteForm').action = "{{ route('bas.employees.delete', '') }}/" + employeeId;
            });
        });
    }
