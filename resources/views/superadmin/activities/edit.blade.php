...
<div class="py-5 px-4">
    <div class="flex justify-between items-center mb-5">
        <h1 class="text-dark tracking-light text-[32px] font-bold leading-tight">Edit Aktivitas</h1>
    ...
    <div class="bg-white rounded-xl border border-border p-6 shadow-sm">
        <form id="activityForm" action="{{ route('superadmin.activities.update', $activity->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <select name="name" 
                            ...
                            @foreach($employees as $employee)
                                <option value="{{ $employee->name }}" 
                                        ...
                                        {{ (old('name', $activity->name) == $employee->name) ? 'selected' : '' }}>
                                    {{ $employee->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div>
                    ...
                    <div class="flex items-center bg-white rounded-md p-2 shadow-sm border border-border">
                        ...
                        <select id="activity_type" 
                            ...
                            <option value="{{ $type }}" {{ (old('activity_type', $selectedActivityType) == $type) ? 'selected' : '' }}>
                                {{ $type }}
                            </option>
                            @endforeach
                        </select>
                ...
                <div id="activity_type_other_container" class="{{ $selectedActivityType === 'Lainnya' ? '' : 'hidden' }}">
                    ...
                    <div class="flex items-center bg-white rounded-md p-2 shadow-sm border border-border">
                        ...
                        <input type="text" 
                               ...
                               value="{{ old('activity_type_other', $activityTypeOther) }}" 
                               ...
                               {{ $selectedActivityType === 'Lainnya' ? 'required' : '' }}>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="flex items-center bg-white rounded-md p-2 shadow-sm border border-border">
                        ...
                        <input type="datetime-local" 
                                id="start_datetime" 
                                name="start_datetime" 
                                value="{{ old('start_datetime', Carbon\Carbon::parse($activity->start_datetime)->format('Y-m-d\TH:i')) }}" 
                                class="w-full bg-transparent border-none outline-none text-dark" 
                                required>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="flex items-center bg-white rounded-md p-2 shadow-sm border border-border">
                        ...
                        <input type="datetime-local" 
                                id="end_datetime" 
                                name="end_datetime" 
                                value="{{ old('end_datetime', Carbon\Carbon::parse($activity->end_datetime)->format('Y-m-d\TH:i')) }}" 
                                class="w-full bg-transparent border-none outline-none text-dark" 
                                required>
                    </div>
                </div>
