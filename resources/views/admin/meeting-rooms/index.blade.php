...
<div class="flex flex-col gap-6 h-full">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-500 mt-1">Lihat dan kelola semua ruang meeting</p>
        </div>
        <button id="btnAddRoom" class="flex cursor-pointer items-center justify-center overflow-hidden rounded-lg h-10 px-5 bg-primary text-white gap-2 text-sm font-bold leading-normal tracking-[0.015em] hover:bg-blue-700 transition-colors shadow-sm">
    ...
    <div class="flex flex-col gap-6 bg-white rounded-lg p-6 shadow-sm">
        ...
        <div class="relative w-full md:w-1/2">
            ...
            <input type="search" id="searchRooms" class="block w-full p-3 ps-10 text-sm text-dark border border-gray-200 rounded-lg bg-gray-50 focus:ring-primary focus:border-primary transition-all shadow-sm" placeholder="Cari ruang meeting...">
        </div>
            ...
            <table class="w-full text-sm text-left">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th scope="col" class="px-5 py-3.5">Nama Ruang</th>
                        <th scope="col" class="px-5 py-3.5">Kapasitas</th>
                        <th scope="col" class="px-5 py-3.5">Deskripsi</th>
                        <th scope="col" class="px-5 py-3.5">Fasilitas</th>
                ...
                <tbody id="roomsTableBody">
                    ...
                    <tr class="border-b border-gray-200 hover:bg-gray-50 room-row">
                        ...
                        <td class="px-5 py-4">{{ $room->capacity ?? 'Tidak diatur' }}</td>
                    ...
                    <tr id="noDataRow" class="{{ count($rooms) > 0 ? 'hidden' : '' }}">
                        <td colspan="6" class="px-5 py-10 text-center">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-gray-500 text-sm">Silakan tambahkan ruang meeting baru</p>
                            </div>
                        </td>
                    </tr>
                </tbody>
...
<div id="addRoomModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50 transition-opacity duration-300">
    <div class="bg-white p-6 rounded-xl shadow-xl max-w-2xl w-full mx-4 transform transition-transform duration-300 scale-100">
        <h3 class="text-xl font-bold text-center mb-4">Tambah Ruang Meeting</h3>
        
        <form action="{{ route($routePrefix.'.meeting_rooms.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                    ...
                    <input 
                        ...
                        id="name" 
                        name="name" 
                        class="w-full p-3 text-sm text-gray-700 border border-gray-300 rounded-lg bg-gray-50 focus:ring-primary focus:border-primary transition-all shadow-sm" 
            ...
            <div class="space-y-2">
                ...
                <textarea 
                    ...
                    rows="4" 
                    class="w-full p-3 text-sm text-gray-700 border border-gray-300 rounded-lg bg-gray-50 focus:ring-primary focus:border-primary transition-all shadow-sm" 
                    placeholder="Deskripsi ruangan, informasi tambahan, dll."
                ></textarea>
            </div>
            ...
            <div class="space-y-2">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div class="flex items-center">
                        <input id="add-projector" type="checkbox" name="facilities[]" value="projector" class="h-4 w-4 text-primary focus:ring-primary rounded">
                        <label for="add-projector" class="ml-2 text-sm text-gray-700">Projector</label>
                    </div>
                    <div class="flex items-center">
                        <input id="add-whiteboard" type="checkbox" name="facilities[]" value="whiteboard" class="h-4 w-4 text-primary focus:ring-primary rounded">
                        <label for="add-whiteboard" class="ml-2 text-sm text-gray-700">Whiteboard</label>
                    </div>
                    <div class="flex items-center">
                        <input id="add-ac" type="checkbox" name="facilities[]" value="ac" class="h-4 w-4 text-primary focus:ring-primary rounded">
                        <label for="add-ac" class="ml-2 text-sm text-gray-700">AC</label>
                    </div>
                    <div class="flex items-center">
                        <input id="add-wifi" type="checkbox" name="facilities[]" value="wifi" class="h-4 w-4 text-primary focus:ring-primary rounded">
...
<div id="editRoomModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50 transition-opacity duration-300">
    <div class="bg-white p-6 rounded-xl shadow-xl max-w-2xl w-full mx-4 transform transition-transform duration-300 scale-100">
        <h3 class="text-xl font-bold text-center mb-4">Edit Ruang Meeting</h3>
        ...
        <form id="editRoomForm" action="" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="space-y-2">
                    <label for="edit-name" class="block text-sm font-medium text-gray-700">Nama Ruang</label>
                    <input 
                        type="text" 
                        id="edit-name" 
                        name="name" 
            ...
            <div class="space-y-2">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div class="flex items-center">
                        <input id="edit-projector" type="checkbox" name="facilities[]" value="projector" class="h-4 w-4 text-primary focus:ring-primary rounded">
                        <label for="edit-projector" class="ml-2 text-sm text-gray-700">Projector</label>
                    </div>
                    <div class="flex items-center">
                        <input id="edit-whiteboard" type="checkbox" name="facilities[]" value="whiteboard" class="h-4 w-4 text-primary focus:ring-primary rounded">
                        <label for="edit-whiteboard" class="ml-2 text-sm text-gray-700">Whiteboard</label>
                    </div>
                    <div class="flex items-center">
                        <input id="edit-ac" type="checkbox" name="facilities[]" value="ac" class="h-4 w-4 text-primary focus:ring-primary rounded">
                        <label for="edit-ac" class="ml-2 text-sm text-gray-700">AC</label>
                    </div>
                    <div class="flex items-center">
                        <input id="edit-wifi" type="checkbox" name="facilities[]" value="wifi" class="h-4 w-4 text-primary focus:ring-primary rounded">
...
<div id="deleteModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50 transition-opacity duration-300">
    <div class="bg-white p-6 rounded-xl shadow-xl max-w-md w-full mx-4 transform transition-transform duration-300 scale-100">
        ...
        <p class="text-gray-600 text-center mb-6">Apakah Anda yakin ingin menghapus ruang meeting ini? Tindakan ini tidak dapat dibatalkan.</p>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        editButtons.forEach(button => {
            button.addEventListener('click', function() {
                ...
                // Set the form action URL with the correct route prefix
                const routePrefix = "{{ $routePrefix }}";
                editRoomForm.action = `/${routePrefix}/meeting-rooms/${roomId}`;
        ...
        deleteButtons.forEach(button => {
            button.addEventListener('click', function() {
                ...
                // Set the form action URL with the correct route prefix
                const routePrefix = "{{ $routePrefix }}";
                deleteForm.action = `/${routePrefix}/meeting-rooms/${roomId}`;
