...
<div class="flex flex-col gap-6 h-full">
    ...
    <div class="flex items-center justify-between">
        <h1 class="text-dark text-2xl font-bold leading-tight tracking-[-0.015em]">Edit Ruang Meeting</h1>
    ...
    <div class="flex flex-col gap-6 bg-white rounded-lg p-6">
        <form action="{{ route('admin.meeting_rooms.update', $room->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-2">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div class="flex items-center">
                        <input id="projector" type="checkbox" name="facilities[]" value="projector" class="h-4 w-4 text-primary focus:ring-primary" {{ in_array('projector', $facilities) ? 'checked' : '' }}>
                        <label for="projector" class="ml-2 text-sm text-dark">Proyektor</label>
                    </div>
                    <div class="flex items-center">
                        <input id="whiteboard" type="checkbox" name="facilities[]" value="whiteboard" class="h-4 w-4 text-primary focus:ring-primary" {{ in_array('whiteboard', $facilities) ? 'checked' : '' }}>
                        <label for="whiteboard" class="ml-2 text-sm text-dark">Papan Tulis</label>
                    </div>
                    <div class="flex items-center">
                        <input id="ac" type="checkbox" name="facilities[]" value="ac" class="h-4 w-4 text-primary focus:ring-primary" {{ in_array('ac', $facilities) ? 'checked' : '' }}>
                        <label for="ac" class="ml-2 text-sm text-dark">AC</label>
                    </div>
                    <div class="flex items-center">
                        <input id="wifi" type="checkbox" name="facilities[]" value="wifi" class="h-4 w-4 text-primary focus:ring-primary" {{ in_array('wifi', $facilities) ? 'checked' : '' }}>
